...
def run_model(model, steps):
    ...
    weights = [len(data)/len(edic[eth]) for eth in t2i] 
...
def call_ethnicity_inference(name, params_dict):
...
def call_ethnicity_inference_batch(names, params_dict):
    model_path = '-'.join([elem+'_'+str(params_dict[elem]) for elem in sorted(params_dict)])+'.pt'
    best_model = torch.load(model_path, map_location=torch.device('cpu'))
    check = [(list(name),'White') for name in names]
...
hyperparams_dict['classifier'] = 'ethnicity'
